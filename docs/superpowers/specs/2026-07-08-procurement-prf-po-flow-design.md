...
| Project reference | Optional `jobId` relation + free-text `projectRef` on PRF and PO. No schema change to `Job`. PM default suggestion comes from a `resolvePmSuggestion(job)` helper (today: job creator; future PM/planner user fields slot in without redesign). **(Revised 2026-07-20 — §14: job linking removed from the PO UI, so the PO Route-to-PM picker has no suggested default. Schema, index and helper are retained; PRFs keep their job picker.)** |
| Supplier acknowledgement | Manual recording by PM/staff: one new status `supplier_accepted` + audited `confirmedDeliveryDate` field (recordable at acceptance or later). No tokenized supplier portal. **(Revised 2026-07-20 — §14: acceptance is an event recordable at any non-terminal issued status, not a gate; `confirmedDeliveryDate` is required at capture.)** |
...
- **Supplier acceptance:** ~~allowed in `sent` only~~ **(Revised 2026-07-20 — §14: recordable at `sent | supplier_accepted | partially_received | fully_received`, advancing the status only from `sent`; `confirmedDeliveryDate` is REQUIRED at capture and revisable on any non-terminal PO.)** Captures acceptance date, `confirmedDeliveryDate`, `supplierAckReference`, notes, optional attachment. `confirmedDeliveryDate` may also be set/updated later — every change is audited as `delivery_date_updated` with metadata `{previousDate, newDate, reason}` (reason prompted in the dialog, optional), so the full revision trail (Friday → Monday → Wednesday) is visible on the PO's audit history for disputes; no separate history table needed. A "Delivery scheduled" badge derives from the field's presence — it is NOT a status, so receiving is never blocked by a missing date.
...
- **New nav section "Purchase Requests"** beside Purchase Orders: list page (status/supplier/warehouse/job filters), create/edit form (supplier, warehouse, ~~job picker +~~ projectRef *(job picker removed — §14)*, required-by date *(§14)*, quote ref/date/validity, IRM line items with quoted prices — same item picker as PO form, attachments, justification), detail page (status timeline, per-status actions: Submit / Approve / Reject-with-reason / Reopen-for-revision / Generate PO / Duplicate-as-revision / Cancel, audit history tab) — mirroring the `PurchaseOrderForm`/`PurchaseOrderDetail` structure. New `purchase-request.service.ts`.
- **PO screens:** source-PRF banner + link, ~~job~~/projectRef display *(job display and picker removed — §14)*, "Route to PM" dialog (user picker, ~~default from `resolvePmSuggestion`~~ — no default, §14), "Record supplier acceptance" dialog (date, confirmed delivery date, supplier ref, notes, attachment), "Update delivery date" action in ~~`supplier_accepted`~~ *(any non-terminal PO — §14)*, new statuses in `poStatus.tsx` (`pm_review` → "PM Review", `supplier_accepted` → "Supplier Accepted", "Delivery scheduled" badge), an "Awaiting my action" filter for PMs (pm_review + pmUserId = me).
...
## 14. Amendment (2026-07-20, post-build review)

Revised with the owner after build. Four changes supersede decisions above; each was made deliberately and is recorded here so the code and this document agree.

- **Job linking is REMOVED from the procurement UI — both PRF and PO** (§2 "Project reference", §9's "job picker + projectRef" on the PRF form and "job/projectRef display" + "default from `resolvePmSuggestion`" on PO screens are superseded). The Job picker is gone from `PurchaseRequestForm` and `PurchaseOrderForm`, and the Job field from `PurchaseOrderDetail`; `listPmCandidates` is no longer called with a `jobId`, so the Route-to-PM picker has **no suggested default** and the PM is chosen manually. `projectRef` (free text) becomes the sole visible project marker on both PRFs and POs.
  - **Backend support is deliberately retained, not deleted**: `PurchaseRequest.jobId` / `PurchaseOrder.jobId`, both `job` relations, `@@index([jobId])`, and `resolvePmCandidates`/`suggestedUserId` all stay. Re-enabling the surface is a frontend-only change.
  - **Existing links survive**: both forms still carry the stored `jobId` through a save rather than silently unsetting it, and conversion still copies the PRF's `jobId` onto the PO ([purchase-request.service.ts:651](../../../backend/src/modules/purchase-request/purchase-request.service.ts)). A link created before this change — or by any future non-UI path — is preserved; it is simply not shown or editable.
- **Supplier acceptance is an EVENT, not a gate** (§2 "Supplier acknowledgement" and §4.2 "allowed in `sent` only" are superseded). Goods routinely arrive before the paperwork, so acceptance is recordable at any non-terminal issued status (`sent | supplier_accepted | partially_received | fully_received`) and only ADVANCES `sent → supplier_accepted`; on the others the receipt status stays the source of truth. Audit metadata carries `statusAtAcceptance` + `statusChanged` so a late acknowledgement is self-evident. `confirmedDeliveryDate` is now **required at capture** (§4.2's "may be set later" is superseded — the warehouse plans against it), and revisable on any non-terminal PO rather than in `supplier_accepted` only.
- **PRF carries a required `requiredByDate`, which becomes the PO's `expectedDeliveryDate`** (new; §3.1 predates the field). It is the ONLY source for that date — never derived from the supplier's lead time, which would look like a commitment nobody made and would contradict whatever the supplier later confirms. Nullable in the schema only because PRFs raised before the field existed have none; conversion refuses those. Conversion also refuses a date that has already **passed** — it builds the PO row directly and would otherwise be the one path able to mint a PO whose delivery date precedes its own order date, which `datesOk` blocks everywhere else (date-only comparison, so same-day conversion is valid).
- **GRN lines take `acceptedQuantity`; `damagedQuantity` is derived** (`received − accepted`) server-side. Deliberately required, never optional-defaulting-to-0: a missing value would otherwise silently mean "everything was damaged" and quietly keep good stock out of inventory.

Also in this build, outside the PRF/PO flow: IRM item suppliers became optional (zero rows allowed, at most one primary — not every item is bought in), and the issued-PDF footer drops the generating user's name (internal audit data the supplier has no use for; it cost the space that was truncating the company's own email) — it remains in the audit log.
