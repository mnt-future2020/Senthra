...
// ("job:accepted"), rejected ("job:rejected"), edited/cancelled/started/completed ("job:updated")
// or deleted ("job:deleted") so the caller can refresh its list — and again after any reconnect,
// to recover events missed while offline. Gated on an authenticated principal; subscribes through
// the shared tab-wide socket (see lib/socket.ts) and tears down on unmount / logout.
const JOB_EVENTS = ["job:new", "job:accepted", "job:rejected", "job:updated", "job:deleted"] as const;
