// The "you already have one of these open" advisory, as a pure function so its wording is testable
// without a DOM (this app's suite is Node-only) — the same split as noticeTone.ts and vanStockLine.ts.
//
...
//  • It named every clashing item AND its reference inline, so its LENGTH grew with the cart. At two
//    items that measured 154 characters and wrapped to two lines; the notice chrome was already the
//    compact tier, so the height was coming entirely from the text.
//
...
export interface OpenLineAdvisory {
  ...
  detail?: string;
  /** Full, untruncated list for the title tooltip when `detail` had to abbreviate. */
  title?: string;
}

// At most this many names are spelled out before the detail line switches to "+N more". Two keeps
// the line inside the notice's width for realistic item names, which is what stops the second line
 ...
 * height.
 */
export function openLineAdvisory(clashes: readonly OpenLineClash[], kind: "restock" | "return"): OpenLineAdvisory | null {
  ...
  const subject = one ? `Open ${noun} already exists for this item.` : `Open ${noun}s already exist for ${clashes.length} of these items.`;
  const action =
    kind === "return" ?
      one ? "You can still include it here."
      : "You can still include them here."
    : one ? "You can still send this one."
    : "You can still send them here.";

  const all = clashes.map(label);
  const detail = all.length <= MAX_NAMED ? all.join(" · ") : `${all.slice(0, MAX_NAMED).join(" · ")} +${all.length - MAX_NAMED} more`;

  return {
    text: `${subject} ${action}`,
    detail,
    // Only worth a tooltip when the line actually hid something.
    title: all.length > MAX_NAMED ? all.join(", ") : undefined,
  };
}
