...
export function JobTitlesView() {
  ...
  const { can } = useAuth();
  // Job titles are employee master-data, so they reuse the user permissions.
  ...
  const [confirm, setConfirm] = React.useState<{ open: boolean; jt: JobTitle | null }>({
    open: false,
    jt: null,
  });
  ...
  const load = React.useCallback(async () => {
    try {
      setJobTitles(await jobTitleService.listJobTitles());
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load job titles.");
    }
  }, []);
  ...
  const add = async () => {
    ...
    setAdding(true);
    try {
      await jobTitleService.createJobTitle(name);
      setNewName("");
      await load();
      pushToast("Job title added.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not add job title.", "alert");
    } finally {
      setAdding(false);
    }
  };

  const startEdit = (d: JobTitle) => {
    setEditingId(d.id);
    setEditName(d.name);
  };
  const cancelEdit = () => {
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      ...
      cancelEdit();
      await load();
      pushToast("Job title renamed.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not rename job title.", "alert");
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  return (
    <div className="flex h-full flex-col gap-5">
      ...
      <div className="shrink-0 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-xs">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h3 className="text-sm font-extrabold text-[var(--ink)]">Job titles</h3>
            <p className="text-xs text-[var(--muted)]">
              The designations you can assign to staff. These power the Job title picker on the user form.
            </p>
          </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xs">
        {loading && jobTitles.length === 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            <Briefcase className="mb-3 h-10 w-10 text-[var(--faint)]" />
            <p className="font-extrabold text-[var(--ink)]">No job titles yet</p>
            <p className="mt-1 text-xs text-[var(--muted)]">
              {canCreate ? "Add your first job title above." : "Ask an administrator to add one."}
            </p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            <Search className="mb-3 h-10 w-10 text-[var(--faint)]" />
            <p className="font-extrabold text-[var(--ink)]">No job titles match</p>
                  ...
                  {editingId === d.id ? (
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {canDelete && (
                          <button
                            onClick={() => setConfirm({ open: true, jt: d })}
      ...
      <ConfirmDialog
        ...
        message={
          <>
            Delete <strong className="text-[var(--ink)]">{confirm.jt?.name}</strong>? Staff already
            assigned to it keep the title on their profile; it just won&apos;t appear in the picker.
          </>
        }
        confirmLabel="Delete"
        onConfirm={doDelete}
        onClose={() => setConfirm({ open: false, jt: null })}
      />
    </div>
  );
}
