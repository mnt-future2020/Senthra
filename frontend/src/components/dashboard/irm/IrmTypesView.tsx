...
export function IrmTypesView() {
  ...
  // Filters derived from the URL — survive a refresh and tab switches.
    ...
    <div className="stack flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xs">
        ...
        {!loading && !error && filtered.length > 0 && (
            <Pagination embedded page={safePage} totalPages={totalPages} total={filtered.length} label="IRM types" onPage={(n) => patch({ page: n > 1 ? String(n) : null }, false)} />
        )}
      </div>
