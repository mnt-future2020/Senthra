...
import * as irmCategoryService from "@/services/irm-category.service";
...
// Settings → IRM Categories: the COMPANY-domain taxonomy (separate from customer
// categories). IRM items pick a category from here. Mirrors the Supplier Types screen.
export function IrmCategoriesView() {
  const { pushToast } = useDashboard();
  const { can } = useAuth();
  const canCreate = can("irm_categories.create");
  ...
  const load = React.useCallback(async () => {
    try {
      setCategories(await irmCategoryService.listIrmCategories());
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load IRM categories.");
    }
  }, []);
  ...
  const add = async () => {
    ...
    setAdding(true);
    try {
      await irmCategoryService.createIrmCategory({ name, description: newDescription.trim() || undefined });
      setNewName("");
      setNewDescription("");
      await load();
      pushToast("IRM category added.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not add IRM category.", "alert");
    } finally {
      setAdding(false);
    }
  };

  const startEdit = (c: IrmCategory) => {
    setEditingId(c.id);
    setEditName(c.name);
  ...
  const saveEdit = async (id: string) => {
    ...
    setSavingEdit(true);
    try {
      await irmCategoryService.updateIrmCategory(id, { name, description: editDescription.trim() });
      cancelEdit();
      await load();
      pushToast("IRM category updated.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not update IRM category.", "alert");
    } finally {
      setSavingEdit(false);
    }
  };

  const toggleStatus = async (c: IrmCategory) => {
    if (togglingId) return;
    setTogglingId(c.id);
    const next = c.status === "active" ? "inactive" : "active";
    try {
      await irmCategoryService.updateIrmCategory(c.id, { status: next });
      await load();
      ...
      <div className="shrink-0 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-xs">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h3 className="text-sm font-extrabold text-[var(--ink)]">IRM Categories</h3>
            <p className="text-xs text-[var(--muted)]">
              The company-domain category list. IRM items pick a category from here.
            </p>
          </div>
        ...
        {canCreate && (
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              placeholder="New IRM category name"
              maxLength={60}
              className={inputCls}
            />
            <input
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xs">
        {loading && categories.length === 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            ...
            <p className="font-extrabold text-[var(--ink)]">No IRM categories yet</p>
        ...
        {!loading && !error && filtered.length > 0 && (
            <Pagination embedded page={safePage} totalPages={totalPages} total={filtered.length} label="IRM categories" onPage={(n) => patch({ page: n > 1 ? String(n) : null }, false)} />
        )}
      </div>
