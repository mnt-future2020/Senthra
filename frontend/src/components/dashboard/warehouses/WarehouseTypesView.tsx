...
// Settings → Warehouse Types: the operational classification master list. Warehouses
// pick a type from here; an in-use type can't be deleted (the API guards it). Mirrors
// the Categories settings screen.
export function WarehouseTypesView() {
  const { pushToast } = useDashboard();
  const { can } = useAuth();
  const canCreate = can("warehouse_types.create");
  ...
  const [confirm, setConfirm] = React.useState<{ open: boolean; type: WarehouseType | null }>({
    open: false,
    type: null,
  });
  ...
  const load = React.useCallback(async () => {
    try {
      setTypes(await warehouseTypeService.listWarehouseTypes());
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load warehouse types.");
    }
  }, []);
  ...
  const add = async () => {
    ...
    setAdding(true);
    try {
      await warehouseTypeService.createWarehouseType({
        name,
      ...
      setNewDescription("");
      await load();
      pushToast("Warehouse type added.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not add warehouse type.", "alert");
    } finally {
      setAdding(false);
    }
  };
  ...
  const saveEdit = async (id: string) => {
    ...
    setSavingEdit(true);
    try {
      await warehouseTypeService.updateWarehouseType(id, {
        name,
        description: editDescription.trim(),
      });
      cancelEdit();
      await load();
      pushToast("Warehouse type updated.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not update warehouse type.", "alert");
    } finally {
      setSavingEdit(false);
    }
  };

  const toggleStatus = async (t: WarehouseType) => {
    if (togglingId) return;
    setTogglingId(t.id);
    const next = t.status === "active" ? "inactive" : "active";
    try {
      await warehouseTypeService.updateWarehouseType(t.id, { status: next });
      await load();
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      // Surfaces the API's in-use guard ("used by N warehouse(s)…") verbatim.
      ...
      <div className="shrink-0 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-xs">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h3 className="text-sm font-extrabold text-[var(--ink)]">Warehouse Types</h3>
            <p className="text-xs text-[var(--muted)]">
              The operational classification list. Warehouses pick a type from here.
            </p>
          </div>
        ...
        {canCreate && (
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              placeholder="New warehouse type name"
              maxLength={60}
              className={inputCls}
            />
            <input
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xs">
        {loading && types.length === 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            <Boxes className="mb-3 h-10 w-10 text-[var(--faint)]" />
            <p className="font-extrabold text-[var(--ink)]">No warehouse types yet</p>
                  ...
                  {editingId === t.id ? (
                    ...
                    <>
                      ...
                      <span className="shrink-0 text-[11px] text-[var(--faint)]">
                        {t.warehouseCount} warehouse{t.warehouseCount === 1 ? "" : "s"}
                      </span>
                      <button
      ...
      <ConfirmDialog
        ...
        message={
          <>
            ...
            by warehouses can&apos;t be deleted.
          </>
        }
        confirmLabel="Delete"
        onConfirm={doDelete}
