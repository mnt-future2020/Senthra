  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-xs sm:flex-row sm:items-center">
        ...
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value as "all" | PoStatus); setPage(1); }} className={selectCls}>
      ...
      {data && data.total > 0 && <Pagination page={data.page} totalPages={data.totalPages} total={data.total} label="purchase orders" onPage={setPage} />}
