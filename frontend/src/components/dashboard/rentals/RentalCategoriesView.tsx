...
// Rentals → Categories: the rental taxonomy, deliberately its own master rather than a share of
// IRM's (one category master per domain — see the design spec). Mirrors the IRM Categories screen.
export function RentalCategoriesView() {
  const { pushToast } = useDashboard();
  const { can } = useAuth();
  const canCreate = can("rental_categories.create");
  const canEdit = can("rental_categories.edit");
  const canDelete = can("rental_categories.delete");
  ...
  const [confirm, setConfirm] = React.useState<{ open: boolean; cat: RentalCategory | null }>({ open: false, cat: null });
  ...
  const load = React.useCallback(async () => {
    try {
      setCategories(await rentalService.listRentalCategories());
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load rental categories.");
    }
  }, []);
  ...
  const add = async () => {
    ...
    setAdding(true);
    try {
      await rentalService.createRentalCategory({ name, description: newDescription.trim() || undefined });
      setNewName("");
      setNewDescription("");
      await load();
      pushToast("rental category added.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not add rental category.", "alert");
    } finally {
      setAdding(false);
    }
  };
  ...
  const saveEdit = async (id: string) => {
    ...
    setSavingEdit(true);
    try {
      await rentalService.updateRentalCategory(id, { name, description: editDescription.trim() });
      cancelEdit();
      await load();
      pushToast("rental category updated.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not update rental category.", "alert");
    } finally {
      setSavingEdit(false);
    }
  };

  const toggleStatus = async (c: RentalCategory) => {
    if (togglingId) return;
    setTogglingId(c.id);
    const next = c.status === "active" ? "inactive" : "active";
    try {
      await rentalService.updateRentalCategory(c.id, { status: next });
      await load();
  ...
  const doDelete = async () => {
    ...
    setDeleting(true);
    try {
      await rentalService.deleteRentalCategory(confirm.cat.id);
      ...
      await load();
      pushToast("rental category deleted.", "success");
    } catch (e) {
  ...
  return (
    <div className="stack flex h-full flex-col">
      <div className="shrink-0 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-xs">
        {/* One row of controls, no heading block — the Categories pill above already names the list,
            and the search + sort read as a toolbar rather than an afterthought pinned to the right.
            Same shape as the Catalogue tab and the IRM list beside it. */}
        ...
        {canCreate && (
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              placeholder="New rental category name"
              maxLength={60}
              className={inputCls}
            />
            <input
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xs">
        {loading && categories.length === 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            <Tags className="mb-3 h-10 w-10 text-[var(--faint)]" />
            <p className="font-extrabold text-[var(--ink)]">No rental categories yet</p>
                  ...
                  {editingId === c.id ? (
                    ...
                    <>
                      <div className="min-w-0 flex-1">
                        ...
                        {c.description && <div className="truncate text-xs text-[var(--muted)]">{c.description}</div>}
                      </div>
                      ...
                      <button
                        ...
                        className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-bold transition-colors ${
                          c.status === "active" ? "bg-[var(--pos)]/15 text-[var(--pos)]" : "bg-[var(--faint)]/15 text-[var(--faint)]"
        ...
        {!loading && !error && filtered.length > 0 && (
            <Pagination embedded page={safePage} totalPages={totalPages} total={filtered.length} label="rental categories" onPage={(n) => patch({ page: n > 1 ? String(n) : null }, false)} />
        )}
      </div>
      ...
      <ConfirmDialog
        ...
        message={
          <>
            Delete <strong className="text-[var(--ink)]">{confirm.cat?.name}</strong>? A category in use by
            items can&apos;t be deleted.
          </>
        }
        confirmLabel="Delete"
        onConfirm={doDelete}
        onClose={() => setConfirm({ open: false, cat: null })}
      />
    </div>
  );
}
