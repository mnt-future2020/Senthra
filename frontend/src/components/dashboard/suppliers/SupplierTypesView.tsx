...
import { Check, Factory, Loader2, Pencil, Plus, Search, Trash2, X } from "lucide-react";

import { useDashboard } from "@/hooks/useDashboard";
import { useAuth } from "@/hooks/useAuth";
import * as supplierTypeService from "@/services/supplier-type.service";
...
// Settings → Supplier Types: the classification master list. Suppliers pick a type from
// here; an in-use type can't be deleted (the API guards it). Mirrors the Warehouse Types
// settings screen.
export function SupplierTypesView() {
  const { pushToast } = useDashboard();
  const { can } = useAuth();
  const canCreate = can("supplier_types.create");
  const canEdit = can("supplier_types.edit");
  const canDelete = can("supplier_types.delete");
  ...
  // Preserves all existing params (incl. ?tab set by SuppliersPanel) and resets to page 1
  // on filter changes unless called with resetPage = false.
  ...
  const [types, setTypes] = React.useState<SupplierType[]>(
    () => supplierTypeService.getCachedSupplierTypes() ?? [],
  );
  ...
  const [confirm, setConfirm] = React.useState<{ open: boolean; type: SupplierType | null }>({
    open: false,
    type: null,
  });
  ...
  const load = React.useCallback(async () => {
    try {
      setTypes(await supplierTypeService.listSupplierTypes());
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load supplier types.");
    }
  }, []);
  ...
  const add = async () => {
    ...
    setAdding(true);
    try {
      await supplierTypeService.createSupplierType({
        name,
      ...
      setNewDescription("");
      await load();
      pushToast("Supplier type added.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not add supplier type.", "alert");
    } finally {
      setAdding(false);
    }
  };

  const startEdit = (t: SupplierType) => {
    setEditingId(t.id);
    setEditName(t.name);
  ...
  const saveEdit = async (id: string) => {
    ...
    setSavingEdit(true);
    try {
      await supplierTypeService.updateSupplierType(id, {
        name,
        description: editDescription.trim(),
      });
      cancelEdit();
      await load();
      pushToast("Supplier type updated.", "success");
    } catch (e) {
      pushToast(e instanceof Error ? e.message : "Could not update supplier type.", "alert");
    } finally {
      setSavingEdit(false);
    }
  };

  const toggleStatus = async (t: SupplierType) => {
    if (togglingId) return;
    setTogglingId(t.id);
    const next = t.status === "active" ? "inactive" : "active";
    try {
      await supplierTypeService.updateSupplierType(t.id, { status: next });
      await load();
  ...
  const doDelete = async () => {
    ...
    setDeleting(true);
    try {
      await supplierTypeService.deleteSupplierType(confirm.type.id);
      ...
      await load();
      pushToast("Supplier type deleted.", "success");
    } catch (e) {
      // Surfaces the API's in-use guard ("used by N supplier(s)…") verbatim.
  ...
  return (
    <div className="flex h-full flex-col gap-5">
      ...
      <div className="shrink-0 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-xs">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h3 className="text-sm font-extrabold text-[var(--ink)]">Supplier Types</h3>
            <p className="text-xs text-[var(--muted)]">
              The classification list. Suppliers pick a type from here.
            </p>
          </div>
        ...
        {canCreate && (
          <div className="mt-3 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              placeholder="New supplier type name"
              maxLength={60}
              className={inputCls}
            />
            <input
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xs">
        {loading && types.length === 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            <Factory className="mb-3 h-10 w-10 text-[var(--faint)]" />
            <p className="font-extrabold text-[var(--ink)]">No supplier types yet</p>
                  ...
                  {editingId === t.id ? (
                    ...
                    <>
                      <div className="min-w-0 flex-1">
                        ...
                        {t.description && (
                          <div className="truncate text-xs text-[var(--muted)]">{t.description}</div>
                        )}
                      </div>
                      <span className="shrink-0 text-[11px] text-[var(--faint)]">
                        {t.supplierCount} supplier{t.supplierCount === 1 ? "" : "s"}
                      </span>
                      <button
      ...
      <ConfirmDialog
        ...
        message={
          <>
            Delete <strong className="text-[var(--ink)]">{confirm.type?.name}</strong>? A type in use
            by suppliers can&apos;t be deleted.
          </>
        }
        confirmLabel="Delete"
        onConfirm={doDelete}
