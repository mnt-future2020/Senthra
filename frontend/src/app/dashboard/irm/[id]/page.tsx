"use client";

import * as React from "react";
import { useParams } from "next/navigation";

import { PermissionGate } from "@/components/auth/PermissionGate";
import { IrmItemDetail } from "@/components/dashboard/irm/IrmItemDetail";
import { FormError, FormPageSkeleton } from "@/components/ui/FormScaffold";
import * as irmService from "@/services/irm.service";
import type { IrmItem } from "@/types/irm";

// :id may be a database id OR an IRM code (the list links by code).
export default function ViewIrmItemPage() {
  const params = useParams();
  const idOrCode = String(params.id);

  return (
    <PermissionGate anyOf={["irm.view"]}>
      <IrmLoader key={idOrCode} idOrCode={idOrCode} />
    </PermissionGate>
  );
}

function IrmLoader({ idOrCode }: { idOrCode: string }) {
  const [item, setItem] = React.useState<IrmItem | null>(null);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);

  React.useEffect(() => {
    let active = true;
    irmService
      .getIrmItem(idOrCode)
      .then((it) => {
        if (active) setItem(it);
      })
      .catch((e) => {
        if (active) setError(e instanceof Error ? e.message : "Could not load this item.");
      })
      .finally(() => {
        if (active) setLoading(false);
      });
    return () => {
      active = false;
    };
  }, [idOrCode]);

  if (loading) return <FormPageSkeleton />;
  if (error || !item) return <FormError message={error ?? "Item not found."} />;
  return (
    <React.Suspense fallback={<FormPageSkeleton />}>
      <IrmItemDetail initial={item} />
    </React.Suspense>
  );
}
