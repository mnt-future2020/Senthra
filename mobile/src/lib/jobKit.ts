...
// may go back: van stock never left a warehouse, so it can be returned anywhere;
// a line with ANY warehouse-issued qty still owes that part to its own warehouse.
...
export function returnLocationNote(line: JobKitLine): string {
  const split = kitLineSourceSplit(line);
  if (split.vanOnly) return "Return at any warehouse";
  if (split.vanQty > 0 && split.warehouseQty > 0) {
    return `Return ×${split.warehouseQty} at ${line.warehouseName ?? "the warehouse"}, ×${split.vanQty} at any warehouse`;
  }
  return `Return at ${line.warehouseName ?? "the warehouse"}`;
}
