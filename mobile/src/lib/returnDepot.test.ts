...
// Ported from the web's returnDepot.test.ts. This is the rule that decides which counter an
// engineer walks to with a hired box, so both surfaces have to reach the same answer from the same
// cart — and a phone is where it is actually used.
import {
  canAddRental,
...
// The option shape is the ONE thing that differs from the web's copy of these expectations: this
// app's picker keys its rows by `key`, the web's <Select> by `value`. Same warehouse id either
// way — the decision being pinned here is WHICH depots are offered, not what the field is called.
const ALL = [
  { key: "wa", label: "London Logistics Hub" },
  { key: "wb", label: "London Fulfillment Centre" },
  { key: "wc", label: "Bristol Depot" },
];
...
describe("returnWarehouseOptions", () => {
  ...
  it("names the depot from the hire when the warehouse list does not carry it", () => {
    ...
    expect(returnWarehouseOptions({ kind: "fixed", ...A }, [])).toEqual([{ key: A.warehouseId, label: A.warehouseName }]);
  });
});
...
describe("effectiveReturnWarehouse", () => {
  ...
  it("never resolves to a warehouse the picker would not offer", () => {
    ...
    for (const depot of [
      ...
      const offered = returnWarehouseOptions(depot, ALL).map((o) => o.key);
