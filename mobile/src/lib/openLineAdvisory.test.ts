...
// Ported from the web's openLineAdvisory.test.ts — the same clash must produce the same sentence on
// both surfaces. One case differs, and it is marked below.
...
describe("openLineAdvisory", () => {
  ...
  describe("length does not grow with the cart", () => {
    ...
    // THE ONE DELIBERATE DIVERGENCE from the web's copy of this rule.
    //
    // The web abbreviates the same way, then hangs the full list off a `title` tooltip for a mouse to
    // hover. There is no hover on a handset, so this copy carries no `title` at all rather than
    // building a string nothing can ever show. The abbreviation itself — which is what an engineer
    // actually reads — is identical, and that is what is pinned here.
    it("abbreviates the detail line past two items, with no tooltip to fall back on", () => {
