...
// Ported from the web's vanStockLine.test.ts — the composite item key and the line payload have to
// mean the same thing on both surfaces, because the server reads them from both.

// `formatHireDate` lives in format.ts here and in vanStockLine.ts on the web — the same function,
// filed differently. Its expectations are pinned either way, which is the point.
import { formatHireDate } from "./format";
import { splitItemKeys, toLinePayload, vanStockItemKey } from "./vanStockLine";
