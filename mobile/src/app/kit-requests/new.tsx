...
import { createKitRequest, searchKitItems } from "@/services/kitRequest.service";
  ...
  return (
    <Screen>
      ...
      {results.map((opt) => {
        ...
        return (
          <Card key={key} onPress={() => (added ? undefined : addOption(opt))} style={added ? s.added : undefined}>
          ...
          <Card key={line.key}>
            <View style={s.lineRow}>
              ...
              <Stepper
                value={line.qty}
                min={1}
                onChange={(next) => setLines((prev) => prev.map((l) => (l.key === line.key ? { ...l, qty: next } : l)))}
              />
            </View>
            <Button
              title="Remove"
