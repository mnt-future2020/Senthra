...
export const getAvailability = asyncHandler(async (req, res) => {
  res.json(await inventoryService.getAvailability(str(req.query.irmItem) ?? "", str(req.query.warehouse) ?? ""));
});
...
export const listTransfers = asyncHandler(async (req, res) => {
  ...
  res.json(await inventoryService.listTransfers({ search: str(search), irmItem: str(irmItem), warehouse: str(warehouse), page: queryInt(page), pageSize: queryInt(pageSize) }));
});
...
export const getInventory = asyncHandler(async (req, res) => {
  res.json({ inventory: await inventoryService.getInventory(param(req, "id")) });
});

// GET /inventory/:id/transactions (ledger, paged)
export const listTransactions = asyncHandler(async (req, res) => {
  res.json(await inventoryService.listInventoryTransactions(param(req, "id"), queryInt(req.query.page) ?? 1, queryInt(req.query.pageSize) ?? 20));
});

// GET /inventory/:id/purchases (purchase history, read-only)
export const listPurchases = asyncHandler(async (req, res) => {
  res.json({ purchases: await inventoryService.listPurchaseHistory(param(req, "id")) });
});
