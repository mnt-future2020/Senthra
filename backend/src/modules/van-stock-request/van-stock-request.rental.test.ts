...
vi.mock("#modules/rental-item/rental-item.repository.js", () => ({
  findById: vi.fn(), findActiveByCode: vi.fn(), findManyByIds: vi.fn(async () => []),
...
vi.mock("#modules/goods-management/goods-management.service.js", () => ({
  ...
  committedByEngineer: vi.fn(async () => ({ irm: new Map(), rental: new Map() })),
}));
...
import { approve, availability, create, fulfil, myHoldings, scanLookup, searchRequestableItems, walkIn } from "./van-stock-request.service.js";
...
beforeEach(() => {
  ...
  vi.mocked(gmService.committedByEngineer).mockResolvedValue({ irm: new Map(), rental: new Map() } as never);
...
describe("myHoldings — what is field-returnable", () => {
  ...
  it("excludes hired kit committed to an active job", async () => {
    ...
    vi.mocked(gmService.committedByEngineer).mockResolvedValue({ irm: new Map(), rental: new Map([[RENTAL, 2]]) } as never);
...
describe("boundaries", () => {
  // Deferred by design — a walk-in skips the review step, which is exactly the step third-party
  // equipment with a deadline and a bill should not skip.
  it("refuses hired kit at the walk-in counter", async () => {
    await expect(
      walkIn({ engineerId: ENG, warehouseId: WH, reason: "counter", priority: "normal", lines: [{ source: "rental", rentalItemId: RENTAL, itemName: "Fibre Tester", qty: 1 }] } as never, admin),
    ).rejects.toThrow();
  });
