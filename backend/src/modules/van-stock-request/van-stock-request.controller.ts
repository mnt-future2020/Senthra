...
export const listMine = asyncHandler(async (req, res) => {
  ...
  const { status, type, createdVia, search, sort, raisedFrom, raisedTo, page, pageSize } = req.query;
  res.json(await vsrService.listMine(actor.id, { status: queryStr(status), type: queryStr(type), createdVia: queryStr(createdVia), search: queryStr(search), sort: queryStr(sort), raisedFrom: queryStr(raisedFrom), raisedTo: queryStr(raisedTo), page: queryInt(page), pageSize: queryInt(pageSize) }));
});
...
export const listAll = asyncHandler(async (req, res) => {
  const { status, type, priority, createdVia, search, sort, warehouseId, engineerId, raisedFrom, raisedTo, page, pageSize } = req.query;
  res.json(
    await vsrService.listAll(actorFrom(req), {
      status: queryStr(status),
      type: queryStr(type),
      priority: queryStr(priority),
      createdVia: queryStr(createdVia),
      search: queryStr(search),
      sort: queryStr(sort),
      warehouseId: queryStr(warehouseId),
